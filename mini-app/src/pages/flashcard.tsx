...
function FlashcardPage() {
  ...
  useEffect(() => {
    ...
    const loadAndStartSession = async (targetDocId: string) => {
      try {
        // Load flashcards from document
        const data = await documentService.getFlashcards(targetDocId);
        setCards(data || []);

        // Start a session for SM-2 tracking
        if (data && data.length > 0) {
          try {
            const session = await studyService.startFlashcard(targetDocId);
            setSessionId(session.session_id);
          } catch (e) {
            console.warn("Could not start flashcard session, SM-2 ratings won't be tracked:", e);
          }
        }
      } catch (e) {
    ...
    if (!id) {
      // Load from first available doc
      documentService.getDocuments()
        .then(docs => {
          if (docs?.length > 0) {
            loadAndStartSession(docs[0].id);
          } else {
            setLoading(false);
          }
        })
